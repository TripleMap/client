...
function getDisplayMeasurements(feature) {
   ...
   // metric calculation
   const line: any = turf.lineString(feature.geometry.coordinates);
...
RadiusMode.onStop = function (state) {
   ...
   if (state.line.isValid()) {
      ...
      let turfLine = turf.lineString(lineGeoJson.geometry.coordinates);
      const turfLineDistance = turf.length(turfLine);
      ...
      const pointWithRadius = {
         ...
         properties: {
            radius: (turfLineDistance * 1000).toFixed(1),
         },
      };
...
RadiusMode.toDisplayFeatures = function (state, geojson, display) {
   ...
   let turfLine = turf.lineString(geojson.geometry.coordinates);
   const radiusInKm = turf.length(turfLine, { units: 'kilometers' });

   const circleFeature: any = createGeoJSONCircle(center, radiusInKm, state.line.id);
